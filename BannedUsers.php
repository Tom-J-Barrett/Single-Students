...
include 'GetBannedUsers.php';
checkAdmin();
?>
<html>

<head>
    <title>Banned Users | Single Students</title>
			...
			<div class="signuptext container">
				<div class="row">
					...
					<div class="col-md-8 form-group">
						<br><b><a href="UserProfileOther.php?user=<?php echo $users[$x];?>"><?php echo ($users[$x]);?></a></b>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="container">
		<div class="signuptext container">
			...
			if ($_SESSION['bPage'] != 0) {
				?>
			<div class="col-xs-6">
				<button  onclick="location.href = 'BannedUsers.php?bPage=<?php echo $x - 5 - ($x%5)?>';" class="btn btn-success index-btn" type="button" name="Previous">Previous</button>
			</div>
			<?php ;
			}
			if ($x != $endPage) {
			?>
			<div class="col-xs-6">
				<button  onclick="location.href = 'BannedUsers.php?bPage=<?php echo $x?>';" class="btn btn-success index-btn" type="button" name="Next">Next</button>
			</div>
			<?php ;
			} ?>
		</div>
